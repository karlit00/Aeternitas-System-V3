...
@section('title', 'Offboarding - ' . $employee->full_name)
...
<div class="min-h-screen bg-gray-50">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="border rounded-lg p-4 hover:bg-gray-50 transition-colors">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex space-x-2">
                        @if($offboardingData['resignation_letter'])
                            <button onclick="openDocumentModal('{{ $employee->full_name }}', 'Resignation Letter', '{{ route('hr.employee-personnel-files.view', [$employee->id, 'offboarding', basename($offboardingData['resignation_letter'])]) }}')"
                               class="text-blue-600 hover:text-blue-800 text-sm font-medium">
                                View
                            </button>
                                <form action="{{ route('hr.employee-personnel-files.delete', [$employee->id, 'offboarding', basename($offboardingData['resignation_letter'])]) }}" 
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="border rounded-lg p-4 hover:bg-gray-50 transition-colors">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex space-x-2">
                        @if($offboardingData['exit_interview_records'])
                            <button onclick="openDocumentModal('{{ $employee->full_name }}', 'Exit Interview Records', '{{ route('hr.employee-personnel-files.view', [$employee->id, 'offboarding', basename($offboardingData['exit_interview_records'])]) }}')"
                               class="text-blue-600 hover:text-blue-800 text-sm font-medium">
                                View
                            </button>
                                <form action="{{ route('hr.employee-personnel-files.delete', [$employee->id, 'offboarding', basename($offboardingData['exit_interview_records'])]) }}" 
                                      method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this file?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-red-600 hover:text-red-800 text-sm font-medium">
        ...
        <div class="bg-white rounded-lg shadow-md p-6 lg:col-span-2">
            ...
            <div class="border rounded-lg p-4 hover:bg-gray-50 transition-colors">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex space-x-2">
                        @if($offboardingData['termination_documentation'])
                            <button onclick="openDocumentModal('{{ $employee->full_name }}', 'Termination Documentation', '{{ route('hr.employee-personnel-files.view', [$employee->id, 'offboarding', basename($offboardingData['termination_documentation'])]) }}')"
                               class="text-blue-600 hover:text-blue-800 text-sm font-medium">
                                View
                            </button>
                                <form action="{{ route('hr.employee-personnel-files.delete', [$employee->id, 'offboarding', basename($offboardingData['termination_documentation'])]) }}" 
                                      method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this file?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-red-600 hover:text-red-800 text-sm font-medium">
        ...
        <div class="bg-white rounded-lg shadow-md p-6 lg:col-span-2">
            ...
            @if($offboardingData['final_payroll_records']->count() > 0)
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($offboardingData['final_payroll_records'] as $payroll)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    {{ $payroll->pay_period_start->format('M j') }} - {{ $payroll->pay_period_end->format('M j, Y') }}
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <a href="{{ route('payrolls.download-payslip', $payroll->id) }}" 
    ...
    <div class="bg-white rounded-lg shadow-md p-6 mt-6">
        ...
        <p class="text-gray-600 mb-4">Upload additional offboarding documents for {{ $employee->full_name }}</p>
        
        <form action="{{ route('hr.employee-personnel-files.upload', [$employee->id, 'offboarding']) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf
