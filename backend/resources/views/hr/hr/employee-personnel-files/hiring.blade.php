...
@section('title', 'Hiring & Onboarding - ' . $employee->full_name)
...
<div class="min-h-screen bg-gray-50">
            ...
            <div class="border rounded-lg p-4 hover:bg-gray-50 transition-colors">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex space-x-2">
                            @if($hiringData['job_application'])
                            <button onclick="openDocumentModal('{{ $employee->full_name }}', 'Job Application', '{{ route('hr.employee-personnel-files.view', [$employee->id, 'hiring', basename($hiringData['job_application'])]) }}')"
                               class="text-blue-600 hover:text-blue-800 text-sm font-medium">
                                View
                            </button>
                            <form action="{{ route('hr.employee-personnel-files.delete', [$employee->id, 'hiring', basename($hiringData['job_application'])]) }}" 
                ...
                <div class="border rounded-lg p-4 hover:bg-gray-50 transition-colors">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex space-x-2">
                            @if($hiringData['resume'])
                                <button onclick="openDocumentModal('{{ $employee->full_name }}', 'Resume/CV', '{{ route('hr.employee-personnel-files.view', [$employee->id, 'hiring', basename($hiringData['resume'])]) }}')"
                                   class="text-blue-600 hover:text-blue-800 text-sm font-medium">
                                    View
                                </button>
                                <form action="{{ route('hr.employee-personnel-files.delete', [$employee->id, 'hiring', basename($hiringData['resume'])]) }}" 
                                      method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this file?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-red-600 hover:text-red-800 text-sm font-medium">
            ...
            <div class="space-y-3">
                ...
                <div class="border rounded-lg p-4 hover:bg-gray-50 transition-colors">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex space-x-2">
                            @if($hiringData['offer_letter'])
                                <button onclick="openDocumentModal('{{ $employee->full_name }}', 'Offer Letter', '{{ route('hr.employee-personnel-files.view', [$employee->id, 'hiring', basename($hiringData['offer_letter'])]) }}')"
                                   class="text-blue-600 hover:text-blue-800 text-sm font-medium">
                                    View
                                </button>
                                <form action="{{ route('hr.employee-personnel-files.delete', [$employee->id, 'hiring', basename($hiringData['offer_letter'])]) }}" 
                                      method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this file?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-red-600 hover:text-red-800 text-sm font-medium">
                ...
                <div class="border rounded-lg p-4 hover:bg-gray-50 transition-colors">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex space-x-2">
                            @if($hiringData['employment_contract'])
                                <button onclick="openDocumentModal('{{ $employee->full_name }}', 'Employment Contract', '{{ route('hr.employee-personnel-files.view', [$employee->id, 'hiring', basename($hiringData['employment_contract'])]) }}')"
                                   class="text-blue-600 hover:text-blue-800 text-sm font-medium">
                                    View
                                </button>
                                <form action="{{ route('hr.employee-personnel-files.delete', [$employee->id, 'hiring', basename($hiringData['employment_contract'])]) }}" 
                                      method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this file?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-red-600 hover:text-red-800 text-sm font-medium">
        ...
        <div class="bg-white rounded-lg shadow-md p-6 lg:col-span-2">
            ...
            <div class="border rounded-lg p-4 hover:bg-gray-50 transition-colors">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex space-x-2">
                            @if($hiringData['onboarding_checklist'])
                            <button onclick="openDocumentModal('{{ $employee->full_name }}', 'Onboarding Checklist', '{{ route('hr.employee-personnel-files.view', [$employee->id, 'hiring', basename($hiringData['onboarding_checklist'])]) }}')"
                               class="text-blue-600 hover:text-blue-800 text-sm font-medium">
                                View
                            </button>
                            <form action="{{ route('hr.employee-personnel-files.delete', [$employee->id, 'hiring', basename($hiringData['onboarding_checklist'])]) }}" 
                                  method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this file?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-red-600 hover:text-red-800 text-sm font-medium">
    ...
    <div class="bg-white rounded-lg shadow-md p-6 mt-6">
        ...
        <p class="text-gray-600 mb-4">Upload additional hiring and onboarding documents for {{ $employee->full_name }}</p>
        
        <form action="{{ route('hr.employee-personnel-files.upload', [$employee->id, 'hiring']) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf
